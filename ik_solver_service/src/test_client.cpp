...
int main(int argc, char **argv)
{
  ...
  {
    for (int j = 0; j < 1; j++){ // In case multiple solutions should be printed
      for (int i = 0; i < 5; i++){
        printf("%f\t",srv.response.joint_angles[i]);
      }
      printf("\nfeasible: %d arm_to_front: %d arm_bended_up: %d gripper_downwards: %d\n\n",srv.response.feasible,srv.response.arm_to_front,srv.response.arm_bended_up,srv.response.gripper_downwards);
    }
  }
  else
  {
  ...
  ros::ServiceClient solve_preferred_pitch_ik_client = n.serviceClient<ik_solver_service::SolvePreferredPitchIK>("solve_preferred_pitch_ik");
  ...
  if (solve_preferred_pitch_ik_client.call(pp_srv))
  {
    for (int j = 0; j < 1; j++){ // In case multiple solutions should be printed
      ...
      printf("\nfeasible: %d arm_to_front: %d arm_bended_up: %d gripper_downwards: %d\n\n",pp_srv.response.feasible,pp_srv.response.arm_to_front,pp_srv.response.arm_bended_up,pp_srv.response.gripper_downwards);
    }
  }
  else
  {
  ...
  ros::ServiceClient solve_preferred_type_ik_client = n.serviceClient<ik_solver_service::SolvePreferredTypeIK>("solve_preferred_type_ik");
  ...
  if (solve_preferred_type_ik_client.call(pt_srv))
  {
    for (int j = 0; j < 1; j++){ // In case multiple solutions should be printed
      ...
      printf("\nfeasible: %d arm_to_front: %d arm_bended_up: %d gripper_downwards: %d\n\n",pt_srv.response.feasible,pt_srv.response.arm_to_front,pt_srv.response.arm_bended_up,pt_srv.response.gripper_downwards);
    }
  }
  else
  {
  ...
  ros::ServiceClient solve_fully_constrained_ik_client = n.serviceClient<ik_solver_service::SolveFullyConstrainedIK>("solve_fully_constrained_ik");
  ...
  if (solve_fully_constrained_ik_client.call(fc_srv))
  {
    for (int j = 0; j < 1; j++){ // In case multiple solutions should be printed
      ...
      printf("\nfeasible: %d arm_to_front: %d arm_bended_up: %d gripper_downwards: %d\n\n",fc_srv.response.feasible,fc_srv.response.arm_to_front,fc_srv.response.arm_bended_up,fc_srv.response.gripper_downwards);
    }
  }
  else
  {
